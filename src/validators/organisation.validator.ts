...
export const createOrganisationValidator: ExpressMiddlewareNext = (request, response, next) => {
  ...
  if (!request.body.org_type || !enums.OrganisationType.includes(request.body.org_type)) {
  ...
  if (!request.body.area_in || !enums.AreaIn.includes(request.body.area_in)) {
  ...
  if (typeof request.body.protected_space !== "boolean") {
    return badRequest(response, Constants.MESSAGES.INVALID_PROTECTED_SPACE.code);
  }
  if (
  ...
  if (!Array.isArray(request.body.operating_hours) || request.body.operating_hours.length === 0) {
  ...
  for (const item of request.body.operating_hours) {
    if ((item.day===undefined || "" )|| (item.startTime===undefined||"") || (item.endTime===undefined||"")) {
      return badRequest(response, Constants.MESSAGES.INVALID_OPERATING_HOURS.code);
    }
  }
...
export const viewOrganisationValidator:ExpressMiddlewareNext = (request,response,next) => {
...
export const updateOrganisationValidator: ExpressMiddlewareNext = (request, response, next) => {

  if (!request.body.id) {
    return badRequest(response, Constants.MESSAGES.ID_REQ.code);
  }
  if (request.body.org_name !== undefined && request.body.org_name.trim() === "") {
    return badRequest(response, Constants.MESSAGES.ORG_NAME_REQUIRED.code);
  }
...
export const deleteOrganisationValidator:ExpressMiddlewareNext = (request,response,next) => {
if(!request.params.id){
  return badRequest(response,Constants.MESSAGES.ID_REQ.code)
}
next()
...
export const listOrganisationValidator:ExpressMiddlewareNext =(request,response,next) => {
